...
**Depends on:** none (Events UX is complete; guest text notes shipped separately and do not block this sprint).
...
| Admin surfaces to theme | Overview/deployment status, tabs, Guests (sessions, **Guest Notes**, history), Guest Types, Events, Photos & Storage, Settings. |
...
- Refactor existing admin rules incrementally: replace hardcoded `#fff`, `#222`, `#ddd` in admin sections with variables. Priority surfaces: tabs, cards, lists, forms, deployment status blocks, photo grid, Events/Guest Types lists, Guest Notes cards.
...
Manual check: Settings, Events, Guests (including Guest Notes), and Photos tabs in dark mode on phone and laptop widths.
...
3. **Polish pass** — deployment status, photo cards, Events tab, Guest Notes, error/success colors in dark mode.
...
- Register guest, Events tab, Guest Notes, and photo preview usable in dark mode.
...
- **Events UX:** complete — admin Events tab, registration picker, gallery re-tag, admin merge UI ([docs/SPRINT_EVENTS_UX.md](SPRINT_EVENTS_UX.md)).
- **Guest text notes:** shipped — guests leave typed notes on Welcome; admin list/delete in Guests tab (not part of this sprint).
- **Sprint queue:** hardening (done) → Events UX (done) → **Admin QoL (this doc)** → UniFi.
